...
I looked at the distirbutions of men and women between desired weight and weight, in my old question I used height as a factor but that was unneeded here so I am answering the original since height and weight have are very correlated. Now I understand that the test I did isn't 100% useful because there isn't a time difference and I am only pulling data from men and women, but I thought it would be good to run tests and show the averages and the quantiles of average weights between men and women and how they only differ on average by around 2 pounds.

New Tools:
I did a permutation test, I used mutate functions, for loops, quantile and mean functions, and ggplot tools that make the graphs easier to read. Similar to david I did it between weight and desired weight, but instead I changed from age to gender/sex.

Answer:
It makes it clear that the differences between men and women are similar, but still difference, most men would rather gain a little bit of weight (possibly in muscle) while most women would rather lose a little bit of weight. This is possibly due to the societal norms of men being more muscular and women being thiner and slimmer. Here in fact we reject the null hypothesis that both genders would rather lose weight. 

```{r}
differenceWomen <- cdc %>% filter(gender == "f") %>% mutate(difW = wtdesire - weight) %>% select(difW)
differenceMen <- cdc %>% filter(gender == "m") %>% mutate(difM = wtdesire - weight) %>% select(difM)
...
ggplot()+
  ...
  ggtitle("Distribution of Desired Weight Difference from Actual Weight for Men")+
  xlab("Desired weight - Actual weight")
...
ggplot()+
  ...
  ggtitle("Distribution of Desired Weight Difference from Weight for Women")+
  xlab("Desired weight - Actual weight")
